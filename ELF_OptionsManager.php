...
class ELF_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'external-links-filter'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'external-links-filter'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'external-links-filter'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'external-links-filter'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'external-links-filter');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'external-links-filter'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'external-links-filter');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'external-links-filter'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'external-links-filter') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function createFormControl($aOptionKey, $aOptionMeta, $savedOptionValue) {
        if (is_array($aOptionMeta) && count($aOptionMeta) >= 2) { // Drop-down list
            ...
            ?>
            <p><input type="checkbox" name="<?php echo $aOptionKey ?>" id="<?php echo $aOptionKey ?>" value="checked" /></p>
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'external-links-filter');
            case 'false':
                return __('false', 'external-links-filter');

            case 'Administrator':
                return __('Administrator', 'external-links-filter');
            case 'Editor':
                return __('Editor', 'external-links-filter');
            case 'Author':
                return __('Author', 'external-links-filter');
            case 'Contributor':
                return __('Contributor', 'external-links-filter');
            case 'Subscriber':
                return __('Subscriber', 'external-links-filter');
            case 'Anyone':
                return __('Anyone', 'external-links-filter');
        }
        return $optionValue;
    }
